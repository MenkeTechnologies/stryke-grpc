...
/// Coerce a JSON value to a bool the way the stryke FFI delivers booleans.
/// stryke's `to_json` cannot emit a JSON boolean, so a flag set in stryke
/// arrives as a `1`/`0` number, a `true`/`false` string, or — for the `\1`/`\0`
/// reference idiom — a `"SCALAR(..)"` string. Treat a real bool, a non-zero
/// number, and any non-empty string except `"0"`/`"false"`/`"SCALAR(0x..0)"`
/// markers as true; everything else (and absent) is false.
...
/// Build an HTTP/2 `:authority` value from parts — the inverse of
/// `parse_authority`. A bare `host` passes through; with a `port` the result is
/// `host:port`, and an IPv6-literal host (one containing `:`, not already
/// bracketed) is wrapped in `[…]` so the host/port colon stays unambiguous
/// (`[::1]:443`). opts: `host` (required), `port` (optional). Returns
/// `{authority, host, port}`. Round-trips through `parse_authority`. Pure.
fn op_build_authority(opts: Value) -> Result<Value> {
    let host = opts
        .get("host")
        .and_then(Value::as_str)
        .map(str::trim)
    ...
    let port = opts.get("port").and_then(Value::as_u64);
    // An IPv6 literal must be bracketed before a port colon can be appended.
    let needs_brackets = host.contains(':') && !host.starts_with('[');
    let bracketed = if needs_brackets {
        format!("[{host}]")
    } else {
        host.to_string()
    };
...
/// Whether a metadata key is an HTTP/2 pseudo-header — a name beginning with `:`
/// (`:method`, `:scheme`, `:path`, `:authority`, `:status`). Pseudo-headers are
/// generated by the transport and must never be set as application metadata; this
/// is the standalone classifier that `is_reserved_key` folds in. `known` is true
/// only for the five request/response pseudo-headers HTTP/2 defines. opts: `key`
/// (required). Returns `{key, pseudo, known}`. Pure.
fn op_is_pseudo_header(opts: Value) -> Result<Value> {
    const PSEUDO: &[&str] = &[":method", ":scheme", ":path", ":authority", ":status"];
    let key = opts
        .get("key")
        .and_then(Value::as_str)
        .ok_or_else(|| anyhow!("missing key"))?;
    let lower = key.to_ascii_lowercase();
    let pseudo = lower.starts_with(':');
    Ok(json!({
        "key": key,
        "pseudo": pseudo,
        "known": PSEUDO.contains(&lower.as_str()),
    }))
}

/// The gRPC status codes a client may safely retry by default, per the gRPC
/// retry design (`grpc/proposal` A6 — `gRPC Retry Design`). Only `UNAVAILABLE`
/// (transient) is universally retryable; the broader set most policies allow adds
/// `CANCELLED`, `DEADLINE_EXCEEDED`, `RESOURCE_EXHAUSTED`, and `ABORTED`. A code
/// reported retryable here is one whose request the caller can replay without
/// changing semantics for an idempotent method.
const RETRIABLE_STATUSES: &[&str] = &[
    "CANCELLED",
    "DEADLINE_EXCEEDED",
    "RESOURCE_EXHAUSTED",
    "ABORTED",
...
/// Whether a gRPC status (by `name` or numeric `code`) is in the retryable set a
/// client may replay for an idempotent method (gRPC retry design A6). `transient`
/// flags `UNAVAILABLE` specifically — the one code retryable for any method, not
/// just idempotent ones. opts: `name` or `code` (one required). Returns
/// `{code, name, retriable, transient}`. Pure.
fn op_retriable_status(opts: Value) -> Result<Value> {
    let (name, code) = resolve_status(&opts)?;
    Ok(json!({
        "code": code,
        "name": name,
        "retriable": RETRIABLE_STATUSES.contains(&name),
        "transient": name == "UNAVAILABLE",
    }))
}

/// Frame a gRPC message into the Length-Prefixed-Message wire form
/// (PROTOCOL-HTTP2): a 1-byte Compressed-Flag, a 4-byte big-endian message
/// length, then the message bytes. The message `payload` is taken base64-encoded
/// (the same `-bin` wire form `encode_bin_value` produces) so arbitrary bytes
/// survive the JSON FFI; the framed result is returned base64-encoded too. opts:
/// `payload` (base64, required), `compressed` (bool — sets the flag byte; the
/// caller is responsible for already-compressed bytes). Returns
/// `{frame, length, compressed}`. Inverse of `unframe_message`. Pure.
fn op_frame_message(opts: Value) -> Result<Value> {
    let payload_b64 = opts
        .get("payload")
        .or_else(|| opts.get("message"))
        .and_then(Value::as_str)
    ...
    let len = u32::try_from(payload.len()).map_err(|_| {
        anyhow!(
            "message length {} exceeds the 4-byte frame limit",
            payload.len()
        )
    })?;
    let compressed = truthy(opts.get("compressed"));
...
/// Parse a gRPC Length-Prefixed-Message frame back into its parts — the inverse
/// of `frame_message`. The base64 `frame` is decoded to a 1-byte Compressed-Flag,
/// a 4-byte big-endian length, then the message bytes; the embedded length is
/// validated against the actual payload. The extracted message is returned
/// base64-encoded (same wire form `frame_message` consumes). opts: `frame`
/// (base64, required). Returns `{compressed, length, payload}`. Pure.
fn op_unframe_message(opts: Value) -> Result<Value> {
    let frame_b64 = opts
        .get("frame")
        .and_then(Value::as_str)
        .ok_or_else(|| anyhow!("missing frame"))?;
    let frame = base64_decode(frame_b64)?;
    if frame.len() < 5 {
        return Err(anyhow!(
            "frame too short: need ≥5 bytes (1 flag + 4 length), got {}",
            frame.len()
        ));
    }
    ...
    if payload.len() != len {
        return Err(anyhow!(
            "frame length prefix {len} does not match payload size {}",
            payload.len()
        ));
    }
    Ok(json!({
...
#[no_mangle]
pub extern "C" fn grpc__build_authority(args: *const c_char) -> *const c_char {
    ffi_call_async(args, |opts| async move { op_build_authority(opts) })
}

#[no_mangle]
pub extern "C" fn grpc__is_pseudo_header(args: *const c_char) -> *const c_char {
    ffi_call_async(args, |opts| async move { op_is_pseudo_header(opts) })
}

#[no_mangle]
pub extern "C" fn grpc__retriable_status(args: *const c_char) -> *const c_char {
    ffi_call_async(args, |opts| async move { op_retriable_status(opts) })
}

#[no_mangle]
pub extern "C" fn grpc__frame_message(args: *const c_char) -> *const c_char {
    ffi_call_async(args, |opts| async move { op_frame_message(opts) })
}

#[no_mangle]
pub extern "C" fn grpc__unframe_message(args: *const c_char) -> *const c_char {
    ffi_call_async(args, |opts| async move { op_unframe_message(opts) })
}
...
mod tests {
    ...
    #[test]
    fn build_authority_inverts_parse_authority() {
        // host + port; IPv6 literal gets bracketed.
        assert_eq!(
            op_build_authority(json!({"host": "api.example.com", "port": 443})).unwrap()
                ["authority"],
            json!("api.example.com:443")
        );
        assert_eq!(
            op_build_authority(json!({"host": "localhost"})).unwrap()["authority"],
            json!("localhost")
        );
        assert_eq!(
            op_build_authority(json!({"host": "::1", "port": 443})).unwrap()["authority"],
            json!("[::1]:443")
        );
        // An already-bracketed host is not double-wrapped.
        assert_eq!(
            op_build_authority(json!({"host": "[::1]", "port": 8080})).unwrap()["authority"],
            json!("[::1]:8080")
        );
        // Round-trips through parse_authority.
        let built = op_build_authority(json!({"host": "2607:f8b0::ef", "port": 9090})).unwrap()
            ["authority"]
            .as_str()
            .unwrap()
            .to_string();
        let parsed = op_parse_authority(json!({"authority": built})).unwrap();
        assert_eq!(parsed["host"], json!("2607:f8b0::ef"));
        assert_eq!(parsed["port"], json!(9090));
        // Missing host errors.
        assert!(op_build_authority(json!({})).is_err());
    }

    #[test]
    fn is_pseudo_header_classifies_http2_pseudo_headers() {
        let a = op_is_pseudo_header(json!({"key": ":authority"})).unwrap();
        assert_eq!(a["pseudo"], json!(true));
        assert_eq!(a["known"], json!(true));
        // Leading colon but not one of the five known names.
        let c = op_is_pseudo_header(json!({"key": ":custom"})).unwrap();
        assert_eq!(c["pseudo"], json!(true));
        assert_eq!(c["known"], json!(false));
        // A normal header is neither.
        let n = op_is_pseudo_header(json!({"key": "content-type"})).unwrap();
        assert_eq!(n["pseudo"], json!(false));
        assert_eq!(n["known"], json!(false));
        assert!(op_is_pseudo_header(json!({})).is_err());
    }

    #[test]
    fn retriable_status_matches_retry_design_set() {
        // UNAVAILABLE is both retriable and transient.
        let u = op_retriable_status(json!({"name": "UNAVAILABLE"})).unwrap();
        assert_eq!(u["retriable"], json!(true));
        assert_eq!(u["transient"], json!(true));
        // By numeric code (14 = UNAVAILABLE).
        assert_eq!(
            op_retriable_status(json!({"code": 14})).unwrap()["retriable"],
            json!(true)
        );
        // ABORTED is retriable but not the single transient code.
        let a = op_retriable_status(json!({"name": "ABORTED"})).unwrap();
        assert_eq!(a["retriable"], json!(true));
        assert_eq!(a["transient"], json!(false));
        // Non-retryable codes.
        assert_eq!(
            op_retriable_status(json!({"name": "INVALID_ARGUMENT"})).unwrap()["retriable"],
            json!(false)
        );
        assert_eq!(
            op_retriable_status(json!({"name": "OK"})).unwrap()["retriable"],
            json!(false)
        );
        // Unknown status rejects.
        assert!(op_retriable_status(json!({"name": "NOPE"})).is_err());
    }

    #[test]
    fn frame_message_round_trips_length_prefixed_message() {
        // "hello grpc" base64-encoded, then framed.
        let payload = base64_encode(b"hello grpc");
        let framed = op_frame_message(json!({"payload": payload.clone()})).unwrap();
        assert_eq!(framed["length"], json!(10));
        assert_eq!(framed["compressed"], json!(false));
        let frame_b64 = framed["frame"].as_str().unwrap().to_string();
        // First byte is the flag (0), next four are the big-endian length (10).
        let raw = base64_decode(&frame_b64).unwrap();
        assert_eq!(raw[0], 0);
        assert_eq!(&raw[1..5], &[0, 0, 0, 10]);
        // unframe recovers every part.
        let un = op_unframe_message(json!({"frame": frame_b64})).unwrap();
        assert_eq!(un["length"], json!(10));
        assert_eq!(un["compressed"], json!(false));
        assert_eq!(un["payload"], json!(payload));
        assert_eq!(base64_decode(payload.as_str()).unwrap(), b"hello grpc");
        // The compressed flag survives the round-trip.
        let cf = op_frame_message(json!({"payload": payload.clone(), "compressed": true})).unwrap();
        assert_eq!(cf["compressed"], json!(true));
        let cf_b64 = cf["frame"].as_str().unwrap().to_string();
        assert_eq!(
            op_unframe_message(json!({"frame": cf_b64})).unwrap()["compressed"],
            json!(true)
        );
        // An empty message frames to a 5-byte header with length 0.
        let empty = op_frame_message(json!({"payload": base64_encode(b"")})).unwrap();
        assert_eq!(empty["length"], json!(0));
        let empty_b64 = empty["frame"].as_str().unwrap().to_string();
        assert_eq!(
            op_unframe_message(json!({"frame": empty_b64})).unwrap()["length"],
            json!(0)
        );
        // A frame shorter than the 5-byte header rejects.
        let short = base64_encode(b"ab");
        assert!(op_unframe_message(json!({"frame": short})).is_err());
        // A length prefix that disagrees with the payload rejects.
        let mut bad = vec![0u8, 0, 0, 0, 99];
        bad.extend_from_slice(b"hi");
        assert!(op_unframe_message(json!({"frame": base64_encode(&bad)})).is_err());
        // The compressed flag arrives as a 1/0 integer from the stryke wrapper
        // (to_json cannot emit a JSON bool); the truthy coercion must accept it.
        assert_eq!(
            op_frame_message(json!({"payload": payload.clone(), "compressed": 1})).unwrap()
                ["compressed"],
            json!(true)
        );
        assert_eq!(
            op_frame_message(json!({"payload": payload, "compressed": 0})).unwrap()["compressed"],
            json!(false)
        );
    }
}
